...
export async function GET(request: Request) {
	...
	if (!apiKey) {
		return NextResponse.json(
			{ error: "EDMTRAIN_API_KEY not set" },
			{ status: 500 },
		);
	}
	...
	if (!apiData.success || !Array.isArray(apiData.data)) {
		return NextResponse.json(
			{ error: "Unexpected EDMTrain response shape" },
			{ status: 502 },
		);
	}
	...
	const rows = apiData.data
		.filter((e: EdmtrainEvent) => e.date)
		.map((e: EdmtrainEvent) => {
			const artistList = e.artistList ?? [];
			const artistNames = artistList.map((a) => a.name).join(", ");
			const name = `${e.name ? `${e.name} ` : ""}${artistNames}`;
			return {
				edmtrain_id: e.id,
				name,
				link: e.link ?? null,
