...
function appUrl(): string {
	return "https://bnbhurtspodcast.com";
...
export async function exchangeYouTubeCode(code: string): Promise<{
	...
	if (!tokenRes.ok)
		throw new Error(`YouTube token exchange failed: ${JSON.stringify(raw)}`);
...
export async function exchangeTikTokCode(
	...
	const raw = await tokenRes.json();
	if (!tokenRes.ok)
		throw new Error(`TikTok token exchange failed: ${JSON.stringify(raw)}`);
...
export async function exchangeMetaCode(code: string): Promise<{
	...
	if (!shortRes.ok)
		throw new Error(
			`Meta short-lived token failed: ${JSON.stringify(shortRaw)}`,
		);
	...
	if (!longRes.ok)
		throw new Error(`Meta long-lived token failed: ${JSON.stringify(raw)}`);
...
export async function exchangeThreadsCode(code: string): Promise<{
	...
	if (!shortRes.ok)
		throw new Error(
			`Threads short-lived token failed: ${JSON.stringify(shortRaw)}`,
		);
	...
	const raw = await longRes.json();
	if (!longRes.ok)
		throw new Error(`Threads long-lived token failed: ${JSON.stringify(raw)}`);
